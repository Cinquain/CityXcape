...
final class BubbleAnimation: UIViewController {
...
extension BubbleAnimation {
    ...
    @objc func startBubble() ->Void{
            ...
            _ = self.generateIntRandomNumber(min: 1, max: 6)
...
struct AnimationView: UIViewControllerRepresentable {
    func makeUIViewController(context: Context) -> BubbleAnimation {
        return BubbleAnimation()
    }
    
    func updateUIViewController(_ uiViewController: BubbleAnimation, context: Context) {
        
    }
    
    typealias UIViewControllerType = BubbleAnimation
